  ...
  try {
    ...
    if (projectState.data.project?.geometryElements && projectState.data.project.geometryElements.length > 0) {
      ...
      const circles = projectState.data.project.geometryElements.filter(elem => 
        elem.type === 'circle' || elem.type === 'hole' || (elem.type === 'ellipse' && elem.data && elem.data.radius)
      );
      ...
      for (let i = 0; i < Math.min(2, circles.length); i++) {
        ...
        // 假设直径为8mm，深度为25mm（实际应从图纸获取）
        diameter = circle.data?.radius ? circle.data.radius * 2 : 
                  circle.data?.diameter ? circle.data.diameter : 8;
        depth = circle.data?.depth ? circle.data.depth : 25; // 默认深度
      ...
      if (selectRes.data.success) {
        ...
        if (defineRes.data.success) {
          ...
          if (gcodeRes.data.success) {
            ...
            // 针对高速钢刀具和低功率机床优化的加工参数
            const targetGCode = `O0005 (CNC程序 - 沉头孔加工程序)
(根据机械制图投影关系设置，X长边方向摆放工件)
(坐标原点G54位于工件右上角)
(使用高速钢刀具，适配低功率机床特点)
G21 (毫米编程)
G40 (刀具半径补偿取消)
...
(机械制图投影关系：图纸正面朝向，X轴为长边方向，Y轴为短边方向)
(从右上角开始，X轴向左为负，Y轴向下为正)
G0 X0. Y0. S400 M03 (主轴正转，400转/分钟，适配低功率机床和高速钢刀具)
G43 Z100. H01 (刀具长度补偿)

(点孔操作 - 使用高速钢中心钻T01)
T01 M06 (换1号刀: 高速钢中心钻φ3mm)
G43 Z100. H01 (刀具长度补偿)
G0 X-${firstHole.x}. Y${firstHole.y}. (定位到第一个孔位置，相对于右上角原点)
G81 G98 Z-2.0 R2.0 F30 (点孔，深度2mm，低进给适配高速钢刀具)
...
(钻孔操作 - 使用高速钢钻头T02)
T02 M06 (换2号刀: 高速钢钻头φ${firstHole.diameter}mm)
G43 Z100. H02 (刀具长度补偿)
G0 X-${firstHole.x}. Y${firstHole.y}. (定位到第一个孔位置)
G83 G98 Z-${hole1DrillDepth}. R2.0 Q1.5 F60 (深孔钻，每次进给1.5mm，低进给适配高速钢刀具，深度${hole1DrillDepth}mm)
...
G83 G98 Z-${hole2DrillDepth}. R2.0 Q1.5 F60 (深孔钻，每次进给1.5mm，低进给适配高速钢刀具，深度${hole2DrillDepth}mm)
G80 (取消固定循环)
G0 Z100. (抬刀到安全高度)

(沉孔操作 - 使用高速钢沉头刀T03)
T03 M06 (换3号刀: 高速钢沉头刀φ12mm，90度)
G43 Z100. H03 (刀具长度补偿)
G0 X-${firstHole.x}. Y${firstHole.y}. (定位到第一个孔位置)
G82 G98 Z-5.0 R2.0 P2000 F40 (沉孔，深度5mm，暂停2秒，低进给适配高速钢刀具)
G0 X-${holePositions.length > 1 ? holePositions[1].x : firstHole.x}. Y${holePositions.length > 1 ? holePositions[1].y : firstHole.y}. (定位到第二个孔位置)
G82 G98 Z-5.0 R2.0 P2000 F40 (沉孔，深度5mm，暂停2秒，低进给适配高速钢刀具)
G80 (取消固定循环)
G0 Z100. (抬刀到安全高度)
...
`;
            ...
            // 保存到文件
            fs.writeFileSync('./mechanical_view_counterbore.nc', targetGCode);
            console.log('NC程序已保存到 mechanical_view_counterbore.nc 文件');
          } else {
...
(钻孔操作 - 使用高速钢钻头T02)
T02 M06 (换2号刀: 高速钢钻头φ${holePositions[0].diameter}mm)
G43 Z100. H02 (刀具长度补偿)
...
(钻孔操作 - 使用高速钢钻头T02)
T02 M06 (换2号刀: 高速钢钻头φ8mm)
G43 Z100. H02 (刀具长度补偿)
G0 X-80. Y60. (定位到第一个孔位置)
G83 G98 Z-29. R2.0 Q1.5 F60 (深孔钻，每次进给1.5mm，低进给适配高速钢刀具，深度29mm)
