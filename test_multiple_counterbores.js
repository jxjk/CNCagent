...
// 沉头孔处理测试 - 验证程序是否能正确识别和处理两个沉头孔
async function testMultipleCounterbores() {
  console.log('=== 沉头孔处理功能测试 ===');
  console.log('目标：验证程序是否能正确识别和处理两个沉头孔');
  ...
  try {
    ...
    console.log('状态已就绪，开始测试沉头孔识别和处理...');
    ...
    try {
      ...
      if (gcodeRes.data.success && gcodeRes.data.gCodeBlocks) {
        ...
        // 保存生成的G代码到测试输出文件
        const testOutput = `; 沉头孔处理测试输出
; 生成时间: ${new Date().toISOString()}
; 测试结果: ${hasBothHoles && hasCounterboreProcess ? '通过' : '未通过'}
...
${fullGCode}`;
        ...
        fs.writeFileSync('./test_multiple_counterbores_output.nc', testOutput);
        console.log('测试输出已保存到 test_multiple_counterbores_output.nc');
        ...
        const overallResult = successCount >= 1 && hasBothHoles && hasCounterboreProcess;
...
'程序能够正确识别和处理两个沉头孔，包括完整的点孔、钻孔、沉孔工艺流程。' : 
...
`;
    ...
    fs.writeFileSync('./COUNTERBORE_TEST_REPORT.md', report);
    console.log('详细测试报告已保存到 COUNTERBORE_TEST_REPORT.md');
  })
  .catch(error => {
