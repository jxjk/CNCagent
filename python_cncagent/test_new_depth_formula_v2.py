        ...
        if 'G83 Z-' in line:
            ...
            if 'Drilling' in line:
                ...
                if depth_match:
                    ...
                    print(f"  找到钻孔深度: {drilling_depth} (来自: {line})")
                    break  # 找到第一个钻孔深度即可
            else:
                # 也检查不带"Drilling"注释的G83命令
                depth_match = re.search(r'G83 Z-([0-9.]+) R2 Q1 F100', line)
                if depth_match and drilling_depth is None:
                    drilling_depth = float(depth_match.group(1))
                    print(f"  找到钻孔深度: {drilling_depth} (来自: {line})")
                    break  # 找到第一个钻孔深度即可
        ...
        if 'G84 Z-' in line:
            ...
            if 'Tapping' in line:
                ...
                if depth_match:
                    ...
                    print(f"  找到攻丝深度: {tapping_depth} (来自: {line})")
                    break  # 找到第一个攻丝深度即可
            else:
                # 也检查不带"Tapping"注释的G84命令
                depth_match = re.search(r'G84 Z-([0-9.]+) R2 F', line)
                if depth_match and tapping_depth is None:
                    tapping_depth = float(depth_match.group(1))
                    print(f"  找到攻丝深度: {tapping_depth} (来自: {line})")
                    break  # 找到第一个攻丝深度即可
