...
## 语义对齐与多模态信息融合规则
- **优先级原则**：用户描述 > 3D模型 > PDF图纸 > 图像 > 默认值
- **一致性验证**：确保多源信息在几何特征、尺寸、位置等方面的一致性
- **缺失信息处理**：当某信息源缺失时，通过其他源或工艺知识进行智能推断
- **冲突解决**：当多源信息存在冲突时，优先采用精度更高的信息源（3D模型 > 2D图纸）
- **语义关联**：建立几何特征与加工工艺之间的语义关联，如圆形特征关联钻孔工艺
- **工艺知识嵌入**：将制造工艺知识嵌入到提示词中，指导AI生成符合实际加工需求的程序
