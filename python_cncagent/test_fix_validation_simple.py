"""
Test parameter confusion fix for CNC Agent
...
def test_diameter_extraction():
    ...
    if outer_diameter == 22.0 and inner_diameter == 14.5:
        print("[PASS] Diameter extraction correct: φ22 as outer diameter, φ14.5 as inner diameter")
    else:
        print(f"[FAIL] Diameter extraction error: Expected outer diameter 22.0, inner diameter 14.5, actual outer diameter {outer_diameter}, inner diameter {inner_diameter}")
    ...
    if actual_outer == expected_outer and actual_inner == expected_inner:
        print("[PASS] Diameter extraction in analysis result is correct")
    else:
        print(f"[FAIL] Diameter extraction in analysis result is incorrect: Expected ({expected_outer}, {expected_inner}), Actual ({actual_outer}, {actual_inner})")
    ...
    if correct_positions == len(expected_positions):
        print("[PASS] Hole position extraction is correct")
    else:
        print(f"[FAIL] Hole position extraction error: Expected {len(expected_positions)} positions, actually correct {correct_positions}")
    ...
    if 94.0 in [pos[0] for pos in hole_positions] and 94.0 != actual_outer:
        print("[PASS] X coordinate 94.0 correctly identified as position not diameter")
    else:
...
def test_counterbore_parameter_extraction():
    ...
    try:
        ...
        if outer_diameter == 22.0 and inner_diameter == 14.5:
            print("[PASS] Counterbore parameter extraction is correct")
        else:
            print(f"[FAIL] Counterbore parameter extraction error: Expected outer diameter 22.0, inner diameter 14.5, Actual ({outer_diameter}, {inner_diameter})")
        
        # Verify hole positions
        expected_positions = [(94.0, -30.0), (94.0, 90.0), (94.0, 210.0)]
        if len(positions) == len(expected_positions):
            print("[PASS] Hole position count is correct")
        else:
            print(f"[FAIL] Hole position count error: Expected {len(expected_positions)}, Actual {len(positions)}")
    ...
    try:
        ...
        if counterbore_features:
            ...
            if outer_dia == 22.0 and inner_dia == 14.5:
                print("[PASS] Diameters in virtual features are correct")
            else:
                print(f"[FAIL] Diameters in virtual features are incorrect: Expected (22.0, 14.5), Actual ({outer_dia}, {inner_dia})")
        else:
            print("[FAIL] Counterbore feature not created")
            
    except Exception as e:
        print(f"[FAIL] Virtual feature creation failed: {e}")
