...
def test_with_actual_system_behavior():
    """
    模拟实际系统的行为，确保修复在各种情况下都能正常工作
    """
    # 使用系统实际输出中的特征数据
    detected_features = [
    ...
    print(f"调整后圆形特征数量: {len(adjusted_circles)}")
    
    # 检查调整后的圆形特征
    print("调整后圆形特征位置:")
    for i, circle in enumerate(adjusted_circles):
        print(f"  {i+1}: {circle['center']} (原位置: {circle.get('original_center', 'unknown')})")
    
    # 检查是否有位置为(0,0)的特征（这可能是坐标原点）
    zero_pos_features = [f for f in adjusted_circles if f['center'] == (0, 0)]
    print(f"位置为(0,0)的圆形特征数量: {len(zero_pos_features)}")
    ...
    if len(actual_counterbore_features) == 3:
        print("✅ 成功：识别到正确的3个沉孔特征")
        
        # 检查是否所有孔位置都不为(0,0)
        zero_pos_counterbores = [f for f in actual_counterbore_features if f['center'] == (0, 0)]
        if len(zero_pos_counterbores) > 0:
            print(f"⚠️  警告：有{len(zero_pos_counterbores)}个沉孔位置为(0,0)")
        else:
            print("✅ 所有沉孔位置都不是(0,0)")
    else:
        print(f"❌ 问题：期望3个沉孔特征，但识别到{len(actual_counterbore_features)}个")
    
    print(f"\n用户期望的极坐标位置: X94.0Y-30.0 Y90.0 Y210.0")
    print(f"实际识别到的位置: {[f['center'] for f in actual_counterbore_features]}")

if __name__ == "__main__":
    test_with_actual_system_behavior()
