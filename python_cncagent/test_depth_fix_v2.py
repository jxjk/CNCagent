...
def test_depth_settings():
    ...
    # 如果上面没找到，尝试查找所有Z-指令
    if drilling_depth is None or tapping_depth is None:
        for line in lines:
            if 'G83 Z-' in line and not 'Drilling' in line and drilling_depth is None:
                depth_match = re.search(r'Z-([0-9.]+)', line)
                if depth_match:
                    drilling_depth = float(depth_match.group(1))
            elif 'G84 Z-' in line and not 'Tapping' in line and tapping_depth is None:
                depth_match = re.search(r'Z-([0-9.]+)', line)
                if depth_match:
                    tapping_depth = float(depth_match.group(1))
