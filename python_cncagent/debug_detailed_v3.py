...
detected_features = [
    ...
    {'shape': 'rectangle', 'center': (992, 1105), 'dimensions': (33, 96), 'confidence': 1.00, 'contour': [], 'bounding_box': (975.5, 1060, 33, 96), 'area': 3168},
...
# 步骤1: 检查原始圆形特征
print(f"\n1. 原始圆形特征数量: {len([f for f in detected_features if f['shape'] == 'circle'])}")
for i, f in enumerate([f for f in detected_features if f['shape'] == 'circle']):
    print(f"   圆形特征 {i+1}: 位置{f['center']}, 半径{f.get('radius', 'N/A')}")

# 步骤2: 检查坐标原点选择
print(f"\n2. 选择坐标基准策略: highest_y")
...
print(f"   调整后的圆形特征:")
for i, f in enumerate([f for f in adjusted_features if f['shape'] == 'circle']):
    print(f"   调整后圆形特征 {i+1}: 位置{f['center']}, 半径{f.get('radius', 'N/A')}")

# 步骤4: 检查调整后的圆形特征
adjusted_circles = [f for f in adjusted_features if f['shape'] == 'circle']
print(f"\n   调整后圆形特征数量: {len(adjusted_circles)}")
...
print(f"   识别到的沉孔特征数量: {len(actual_counterbore_features)}")
...
print(f"   生成的G代码中孔加工指令数量: {len(hole_lines)}")
...
if len(actual_counterbore_features) != 3:
    print(f"❌ 问题: 期望3个沉孔特征，但识别到{len(actual_counterbore_features)}个")
    
    if len(actual_counterbore_features) == 0:
        # 检查是否是因为坐标转换导致所有圆形特征位置相同
        unique_positions = set(f['center'] for f in adjusted_circles)
        print(f"   调整后圆形特征中唯一位置数: {len(unique_positions)}")
        if len(unique_positions) == 1:
            print("   🔍 发现问题: 坐标转换后所有圆形特征位置相同")
            
            # 检查原始圆形特征位置
            original_circles = [f for f in detected_features if f['shape'] == 'circle']
            original_positions = set(f['center'] for f in original_circles)
            print(f"   原始圆形特征中唯一位置数: {len(original_positions)}")
        else:
            print(f"   坐标转换后圆形特征位置正常，各有{len(unique_positions)}个不同位置")
else:
    print(f"✅ 成功: 识别到正确的{len(actual_counterbore_features)}个沉孔特征")
